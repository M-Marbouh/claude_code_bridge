...
- **Session resets belong to the user.** Do not run `autonew codex` on your own initiative — the user refreshes Codex manually. An accumulating Codex session increases the context billed on subsequent `ask` calls, so keep each brief bounded to minimize its marginal cost.
<!-- CCB_CONFIG_END -->

...
class CodexAdapter(BaseProviderAdapter):
    ...
    def _wait_for_delivery(
        ...
        while True:
            ...
            role, text, _phase = event
            if role == "user" and f"{REQ_ID_PREFIX} {task.req_id}" in text:
                anchor_seen = True
                anchor_ms = _now_ms() - started_ms
                break

        cancelled = bool(task.cancelled or (task.cancel_event and task.cancel_event.is_set()))
        if cancelled:
        ...
        log_path_value = state.get("log_path") if isinstance(state, dict) else None
    ...
    def handle_task(self, task: QueuedTask) -> ProviderResult:
        ...
        if req.delivery_only:
            prompt = wrap_codex_delivery_prompt(req.message, task.req_id)
        else:
        ...
        if req.delivery_only:
            ...
            _write_log(
                f"[INFO] delivered provider=codex req_id={task.req_id} exit={result.exit_code} "
                f"anchor={result.anchor_seen}"
            )
            return result

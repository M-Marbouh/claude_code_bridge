...
Status: agreed scope; implementation not started. This document is the execution source of truth until completion. It supersedes the June 2026 multi-instance plans for this feature.
...
CCB creates no native-subagent pane, registry identity, queue identity, role, or receipt sender identity. If an internal agent issues an ask, attribute it to the owning top-level live session only when that ownership is verified. Inherited pane variables alone are not assumed or sufficient proof; Phase 1 must test actual caller metadata, including sandbox/background execution and missing/conflicting values. Without verified ownership, reject the ask rather than guess, create an identity, or attribute it to the other paired session. Alternatively the owning top-level session issues the ask itself. Replies return to that top-level session, not the internal agent. Concurrent internal asks use distinct task IDs and the existing top-level routing rules.
...
| Native switch observation | Destination gate and caller-delivery policy evaluation remain open. Save-first universal; save-only final fallback after active evidence evaluation, not an early default. |
| Helper transcript collision | Peer-reported Codex 0.153.4 anchor collision; scanner eligibility omission verified in source. Misread not reproduced; blocking destination regression fixture required. |
| Native internal asks | Top-level attribution required only with verified ownership; actual environment/identity inheritance not empirically established. |

...
def test_cmd_start_rejects_second_instance_in_same_directory(monkeypatch, tmp_path: Path, capsys) -> None:
    ccb = _load_ccb_module()
    (tmp_path / ".ccb").mkdir()
    lock_file = tmp_path / "ccb.lock"
    lock_file.write_text("4242\n", encoding="utf-8")

    class _HeldLock:
        def __init__(self, provider: str, timeout: float, cwd: str) -> None:
            assert provider == "ccb"
    ...
    monkeypatch.setattr(ccb, "ProviderLock", _HeldLock)
    monkeypatch.setattr(ccb, "load_registry_by_project_id", lambda *_args: None)

    rc = ccb.cmd_start(SimpleNamespace(providers=["codex"]))

    assert rc == 2
    err = capsys.readouterr().err
    assert "Another ccb instance is already running" in err
    assert "Only one ccb instance is allowed per directory" in err


def test_cmd_start_rejects_provider_instance_without_falling_back_to_config(monkeypatch, tmp_path: Path, capsys) -> None:
    ccb = _load_ccb_module()
    (tmp_path / ".ccb").mkdir()
    ...
    rc = ccb.cmd_start(SimpleNamespace(providers=["codex:worker"]))

    assert rc == 2
    assert "invalid provider(s): codex:worker" in capsys.readouterr().err
